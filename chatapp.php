...
<link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/tailwindcss/1.8.10/tailwind.min.css'><link rel="stylesheet" href="chatstyle.css">

</head>
<body>
<nav class="navbar">
        <ul class="nav-list">
            <li><a href="chatapp.php">Home</a></li>
            <li><a href="lawyer_chat.php">Chat</a></li>
            <li><a href="cases.php">Cases</a></li>
            <li><a href="lawyerprofile.php">Profile</a></li>
...
<body class="flex w-screen h-screen text-gray-700" >
	...
	<div class="flex flex-col flex-grow">
		<div class="flex items-center flex-shrink-0 h-16 bg-white border-b border-gray-300 px-4">
			<div>
				<h1 class="text-sm font-bold leading-none">Community</h1>
		...
		<?php
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
						...
						<div class="h-10 w-10 rounded flex-shrink-0 bg-gray-300"><img class="card-img-bottom d-block radius-image" src="data:image/jpeg;base64,' . base64_encode($image) . '" alt="Card image cap">
						</div>
						<div class="ml-2">
							<div class="-mt-1">
								<span class="text-sm font-semibold">' . $name . '</span>
								<span class="ml-1 text-xs text-gray-500">' . $formattedTime . '</span>
							</div>
		...
		<div class="h-12 bg-white px-4 pb-4">
			<form action="communitybk.php" method="POST">
			<div class="flex items-center border-2 border-gray-300 rounded-sm p-1">
				...
				<textarea class="flex-grow text-sm px-3 border-l border-gray-300 ml-1" name="message" style="resize: none;" placeholder="Today's Feed" cols="" rows="1"></textarea>
	...
	<div class="flex flex-col flex-shrink-0 w-1/4 max-w-xs border-l border-gray-300 bg-gray-100">
		<div class="flex items-center h-16 border-b border-gray-300 px-4">
			<div class="">
				<h2 class="text-sm font-semibold leading-none">Notification</h2>
		...
		<?php
                ...
                $sql = "SELECT us.image, ap.ap_name, ap.case_name, ap.content, ap.id, ap.ap_date, ap.ap_time  FROM appointment ap INNER JOIN user us ON ap.user_id = us.user_id where ap.res_name = '$law_name' AND ap.status='No Data'  ";
                $result = $conn->query($sql);
				...
				<div class="h-10 w-10 rounded flex-shrink-0 bg-gray-300"><img style="height:40px;" src="data:image/jpeg;base64,' . base64_encode($image) . '" alt="Card image cap"></div>
				<div class="ml-2">
					<div class="-mt-1">
						<span class="text-sm font-semibold">'.$name.'</span>
						<span class="ml-1 text-xs text-gray-500">'.$ap_date.' | ' . $formattedTime . '</span>
					</div>
					<p class="text-sm"> <b>Case:</b> '.$case.' </p>

					<p><a href="rejectbk.php?id='.$id.'"><button class="button-65" role="button">Reject</button></a> <a href="acceptbk.php?id='.$id.'"><button class="button-65" role="button">Accept</button> </a> </p>
					<p></p>
				</div>
			</div></a>';
...
<!-- partial -->
  <script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.5.1/jquery.min.js'></script><script  src="chatscript.js"></script>
